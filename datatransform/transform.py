...
def feature_example(label, domain, feature):
    feature_des = {
...
class DataTransform(object):
    ...
    def _read(self, name=None, header=None, sep=None, label_index=""):
        ...
        self.domain1_data = pd.read_table(self.data_path1, names=name, header=header, sep=sep)
        self.domain2_data = pd.read_table(self.data_path2, names=name, header=header, sep=sep)
        self.data = pd.concat([self.domain1_data, self.domain2_data], ignore_index=True)
    ...
    def random_split(self, data, ratio=[]):
        ...
        train_data = data.sample(frac=ratio[0], replace=False,
                                      axis=0, random_state=self.seed)
        left_data = data[~data.index.isin(train_data.index)]
    ...
    def transform_tfrecord(self, data, record_path, flag, records=5e6, label_index="", domain_id=0):
        ...
        while records * part <= data.shape[0]:
            ...
            for index, row in tmp_data.iterrows():
                ...
                domain = float(domain_id)
                # if oov:
                # continue
                tf_writer.write(feature_example(label, domain, feature))
                pbar.update(1)
                instance_num += 1
            tf_writer.close()
            pbar.close()
