...
from modules import multiMask3
...
parser.add_argument("--scaling", type=float, default=1, help="mask scaling value")
parser.add_argument("--final_temp", type=float, default=100, help="final temperature")
parser.add_argument("--init_thre", type=float, default=0.95, help="init threshold")
...
parser.add_argument("--lambda1", type=float, default=3, help="share loss rate")
parser.add_argument("--lambda2_s", type=float, default=1e-8, help="regularization rate")
parser.add_argument("--lambda2", type=float, default=2e-9, help="regularization rate")
parser.add_argument("--lambda3", type=float, default=5e-9, help="regularization rate")
...
class Trainer(object):
    def __init__(self, opt):
        ...
        self.network = multiMask3.getModel(opt["model"], opt["model_opt"]).to(self.device)
        self.criterion = torch.nn.BCEWithLogitsLoss(reduction='none')
        self.optim = multiMask3.getOptim(self.network, opt["optimizer"], self.lr, self.l2)
    ...
    def train_on_batch(self, label, data, domain, retrain=False):
        ...
        logit0, logit1, logit2, logit0s, logit1s, logit2s = self.network(data)
        ...
        logloss2s = self.criterion(logit2s, label)
        if not retrain:
            regloss1_s = self.reg_lambda2_s * self.network.reg1_s()
            regloss1_i = self.reg_lambda2 * torch.mean(self.network.reg1_i() * torch.eq(domain, 0).type(torch.long))
            regloss1_j = self.reg_lambda2 * torch.mean(self.network.reg1_j() * torch.eq(domain, 1).type(torch.long))
            regloss1_k = self.reg_lambda2 * torch.mean(self.network.reg1_k() * torch.eq(domain, 2).type(torch.long))
            regloss2_i = self.reg_lambda3 * torch.mean(self.network.reg2_i() * torch.eq(domain, 0).type(torch.long))
            regloss2_j = self.reg_lambda3 * torch.mean(self.network.reg2_j() * torch.eq(domain, 1).type(torch.long))
            regloss2_k = self.reg_lambda3 * torch.mean(self.network.reg2_k() * torch.eq(domain, 2).type(torch.long))
            sharelogloss = (torch.mean(logloss0s) + torch.mean(logloss1s) + torch.mean(logloss2s)) / 3
            domainlogloss = torch.mean(logloss0 * torch.eq(domain, 0).type(torch.long) + logloss1 * torch.eq(domain, 1).type(torch.long) + logloss2 * torch.eq(domain, 2).type(torch.long))
            loss = self.share_lambda1 * sharelogloss + domainlogloss + regloss1_s + regloss1_i + regloss1_j + regloss1_k + regloss2_i + regloss2_j + regloss2_k
        else:
            sharelogloss = (torch.mean(logloss0s) + torch.mean(logloss1s) + torch.mean(logloss2s)) / 3
            domainlogloss = torch.mean(logloss0 * torch.eq(domain, 0).type(torch.long) + logloss1 * torch.eq(domain, 1).type(torch.long) + logloss2 * torch.eq(domain, 2).type(torch.long))
            loss = self.share_lambda1 * sharelogloss + domainlogloss
        loss.backward()
    ...
    def search(self):
        ...
        self.mask_weight_k = self.network.mask_embedding.mask_weight_k
        self.init_weight = self.network.mask_embedding.init_weight
        ...
        mask_k = self.network.mask_embedding.mask_weight_k.detach().cpu().numpy()
        s = (mask_s > 0).astype(int)
        i = (mask_i > 0).astype(int)
        j = (mask_j > 0).astype(int)
        k = (mask_k > 0).astype(int)
        s_i = ((mask_s > 0) & (mask_i > 0)).astype(int)
        s_j = ((mask_s > 0) & (mask_j > 0)).astype(int)
        s_k = ((mask_s > 0) & (mask_k > 0)).astype(int)
        self.logger.info("share_remain:{} | spec_i_remain:{} | spec_j_remain:{} | spec_k_remain:{}".format(np.sum(s), np.sum(i-s_i), np.sum(j-s_j), np.sum(k-s_k)))
        self.logger.info("spec_i & spec_j:{} | spec_i & spec_k:{} | spec_j & spec_k:{}".format(np.sum((i-s_i) & (j-s_j)), np.sum((i-s_i) & (k-s_k)), np.sum((j-s_j) & (k-s_k))))
    ...
    def train(self, epochs):
        ...
        cur_auc = 0.0
        early_stop = False
        self.optim = multiMask3.getOptim(self.network, "adam", self.lr, self.l2)[:1]
        rate_s, rate_i, rate_j, rate_k = self.network.compute_remaining_weights()

        self.logger.info("-----------------Begin Train-----------------")
        self.logger.info("Ticket:{t}".format(t=self.network.ticket))
        self.logger.info(
            "Feature s remain:{rate_s:.6f} | Feature i remain:{rate_i:.6f} | Feature j remain:{rate_j:.6f} | Feature k remain:{rate_k:.6f}".format(
                rate_s=rate_s, rate_i=rate_i, rate_j=rate_j, rate_k=rate_k))
        ds = self.dataloader.get_train_data("train", batch_size=self.bs)
        ...
        if not early_stop:
            ...
            test_auc1, test_loss1 = self.evaluate_test("test", "1")
            test_auc2, test_loss2 = self.evaluate_test("test", "2")
            self.logger.info("Final Test AUC0: {test_auc:.6f}, Test Loss0: {test_loss:.6f}".format(test_auc=test_auc0,
                                                                                                 test_loss=test_loss0))
            self.logger.info("Final Test AUC1: {test_auc:.6f}, Test Loss1: {test_loss:.6f}".format(test_auc=test_auc1,
                                                                                                 test_loss=test_loss1))
            self.logger.info("Final Test AUC2: {test_auc:.6f}, Test Loss2: {test_loss:.6f}".format(test_auc=test_auc2,
